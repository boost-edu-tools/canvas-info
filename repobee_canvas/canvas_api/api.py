...
class CanvasAPI:
    ...
    def overrides(self, course_id, assignment_id):
        ...
        return [
            o.extend({COURSE_ID: course_id})
            for o in self.__get(
                {COURSES: course_id, ASSIGNMENTS: assignment_id, OVERRIDES: EMPTY}
            )
        ]

    def assignments_per_course(self, course_id):
        """Get the assignments for a course"""
        return self.__get({COURSES: course_id, ASSIGNMENTS: EMPTY})

    def submissions_per_course(self, course_id, assignment_id):
        """Get submissions for an assignment"""
        return self.__get(
            {COURSES: course_id, ASSIGNMENTS: assignment_id, SUBMISSIONS: EMPTY},
    ...
    def submissions_per_section(self, section_id, assignment_id):
        ...
        return self.__get(
            {SECTIONS: section_id, ASSIGNMENTS: assignment_id, SUBMISSIONS: EMPTY},
            params=GROUPED_SUBMISSION,
        )
    ...
    def section(self, course_id, section_id):
        """Get section"""
        return self.__get(
            {COURSES: course_id, SECTIONS: section_id}, params=ALL_STUDENTS
        )
    ...
    def add_comment_to_submission(
        self, course_id, assignment_id, user_id, msg, file_path=None
    ):
        """Add comment to submission"""
        comment = {COMMENT_TEXT_COMMENT: msg, COMMENT_GROUP_COMMENT: True}
    ...
    def delete_comment_from_submission(
        self, course_id, assignment_id, user_id, comment_id
    ):
    ...
    def submit_url(
        self, course_id, assignment_id, user_id, url, msg=None, submitted_at=None
    ):
        """Submit URL"""
        submission = {
        ...
        return self.__submit(course_id, assignment_id, submission, submitted_at, msg)

    def submit_text(
        self, course_id, assignment_id, user_id, text, msg=None, submitted_at=None
    ):
        """Submit text"""
        submission = {
    ...
    def submit_file(
        self, course_id, assignment_id, user_id, file_path, msg=None, submitted_at=None
    ):
        """Submit file"""
        file_id = self.__upload(
            {
    ...
    def __upload(self, components, path):
        ...
        upload_config = self._session.post(
            url, json={NAME: filename, SIZE: filesize}
        ).json()
        ...
        upload = self._session.post(
            upload_url, json=upload_params, files=files, headers={}
        )
    ...
    def __submit(
        self, course_id, assignment_id, submission={}, submitted_at=None, msg=None
    ):
