...
def CreateStudentsFiles(
    ...
    def warn(msg: str):
        if message_callback:
            message_callback(f"WARNING: {msg}")
        ...
        for course in courses:
            if course.id == canvas_course_id:
                ...
                canvas_git_mapping_table = canvas_git_map_table_wizard(
                    course, progress_callback, message_callback
                )
                ...
                if canvas_git_mapping_table.empty():
                    ...
                    if students_teammates_file:
                        canvas_git_mapping_table.writeTeammatesExcel(students_teammates_file)
                        inform(f"Created students info Teammates Excel file: {students_teammates_file}")
...
def create_yaml_file(
    ...
    if student_member_option == "(email, gitid)":
        write_yaml_file(
            students_yaml_file,
            group_submissions,
            include_group,
    ...
    elif student_member_option == "email":
        write_yaml_file(
            students_yaml_file,
            group_submissions,
            include_group,
...
def write_yaml_file(
    ...
    sorted_group_submissions = {
        k: v for k, v in sorted(group_submissions.items(), key=lambda x: x[1][GROUP].name)
    }
